...
public final class CPLC {
    ...
    public static CPLC fromBytes(byte[] data) {
        ...
        if (data.length < 0x2A) {
            throw new IllegalArgumentException("Input can't be valid CPLC if length is only %02X!".formatted(data.length));
        }
    ...
    public static Optional<LocalDate> toRelativeDate(final byte[] v, final LocalDate now) {
